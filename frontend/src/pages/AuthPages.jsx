      ...
      <section className="relative z-10 mx-auto grid min-h-screen w-full max-w-7xl items-center gap-8 px-4 py-6 sm:px-6 sm:py-8 lg:grid-cols-[1fr_460px] lg:px-8">
        <div className="max-w-2xl">
          <div className="mb-6 inline-flex items-center gap-2 rounded-md border border-white/15 bg-white/10 px-3 py-2 text-xs font-bold uppercase tracking-wide text-blue-50 shadow-sm backdrop-blur">
            <ShieldCheck size={16} />
            Enterprise transcription operations
          </div>
          <h2 className="text-3xl font-bold leading-tight sm:text-5xl">
            Secure audio review and transcription control.
          </h2>
          <p className="mt-5 max-w-xl text-base leading-7 text-slate-100 sm:text-lg">
            Manage audio files, transcript work, reviewer feedback, and team handoffs from one focused workspace.
          </p>
          <div className="mt-7 grid max-w-2xl gap-3 sm:grid-cols-3">
            {[
              ['Audio timeline', AudioLines],
              ['Role-based access', LockKeyhole],
              ['Reviewer workflow', BadgeCheck]
            ].map(([label, Icon]) => (
          ...
          <div className="mt-4 grid max-w-2xl gap-3 sm:grid-cols-2">
            <div className="rounded-md border border-white/15 bg-slate-950/30 p-4 backdrop-blur">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-300">Workflow</p>
              <p className="mt-2 text-sm font-semibold text-white">Upload, assign, transcribe, review, approve</p>
            </div>
            <div className="rounded-md border border-white/15 bg-slate-950/30 p-4 backdrop-blur">
              <div className="flex items-center gap-2 text-slate-300">
                <Clock3 size={16} />
                <p className="text-xs font-bold uppercase tracking-wide">Status visibility</p>
              </div>
              <p className="mt-2 text-sm font-semibold text-white">Track queues and decisions in real time</p>
            </div>
          </div>
        </div>
...
export function ForgotPassword() {
  ...
  return <AuthShell title="Password recovery"><form onSubmit={submit} className="space-y-5"><div><label>Login ID</label><input className="mt-1.5" type="text" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required /></div><button className="btn-primary h-11 w-full">Request reset help</button>{message && <p className="rounded-md border border-emerald-100 bg-emerald-50 p-3 text-sm font-medium text-emerald-700">{message}</p>}<Link className="block text-center text-sm font-medium text-primary" to="/login">Back to login</Link></form></AuthShell>;
}
